...
Despite this, humans are very good at manipulating objects and the necessary changes in timing and muscle tensions to do so are calculated with ease, even if the object has never been encountered before. 
This can be largely attributed to the plasticity of neural connections in the motor control area of the brain.
...
When the precise environment that the robot will operate in is not fully known, it is useful for any control system that it uses to be able to handle unknown environments.
...
Aligning the arms along the body x and y axes gives the diagonal inertia matrix in Equation XXX. Due to symmetry, $Ixx = Iyy$.
...
Where k is the lift constant, b is the drag constant, and IM is the moment of inertia of the rotor.
...
Combining the torques of all four rotors leads to Equation XXX, where the vector tau represents the torques across each of the principal body axes (tau-? for roll, tau-? for pitch, and tau-? for yaw). 
...
Where the adaptive controller really excels is when there are nonlinear external forces being applied
